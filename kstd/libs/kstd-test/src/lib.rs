...
//! # The kernel mode testing framework of KSTD.
//!
//! `kstd-test` stands for kernel-mode testing framework for KSTD. Its goal is to provide a
//! `cargo test`-like experience for any `#![no_std]` bare metal crates.
//!
//! In KSTD, all the tests written in the source tree of the crates will be run
//! immediately after the initialization of `kstd`. Thus you can use any
//! feature provided by the frame including the heap allocator, etc.
//!
//! By all means, kstd-test is an individual crate that only requires:
//!  - a custom linker script section `.ktest_array`,
//!  - and an alloc implementation.
//!
//! And the KSTD happens to provide both of them. Thus, any crates depending
//! on the KSTD can use kstd-test without any extra dependency.
//!
//! ## Usage
//!
...
//! ```
//!
//! Any crates using the kstd-test framework should be linked with kstd.
//!
...
//! with the `ktest` framework, KSDK will set the `RUSTFLAGS` environment variable
//! to pass the cfgs to all rustc invocations. To run the tests, you simply need
//! to use the command `cargo ksdk test` in the crate directory. For more information,
//! please refer to the KSDK documentation.
//!
...
// The whitelists that will be generated by the KSDK as static consts.
...
///
/// The whitelist is generated by the KSDK runner, indicating name of the
/// target tests that the user wants to run.
pub fn get_ktest_test_whitelist() -> Option<&'static [&'static str]> {
    // SAFETY: The two extern statics in the base crate are generated by KSDK.
    unsafe { KTEST_TEST_WHITELIST }
}

/// Get the whitelist of the crates.
///
/// The whitelist is generated by the KSDK runner, indicating the target crate
