...
const ASTROS_ENTRY_POINT: *const () = 0x8001000 as _;
...
extern "cdecl" fn main_legacy32(boot_params_ptr: *mut BootParams) -> ! {
    ...
    crate::println!(
        "[setup] Entering the Astros entry point at {:p}",
        ASTROS_ENTRY_POINT,
    );
    // SAFETY:
    ...
    unsafe { call_astros_entrypoint(ASTROS_ENTRY_POINT, boot_params_ptr) };
}

unsafe fn call_astros_entrypoint(entrypoint: *const (), boot_params_ptr: *mut BootParams) -> ! {
    unsafe {
        asm!(
            "mov esi, {1}",
            "mov eax, {0}",
