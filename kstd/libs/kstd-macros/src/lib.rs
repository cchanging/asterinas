...
pub fn main(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote!(
        #[cfg(not(ktest))]
        #[no_mangle]
        extern "Rust" fn __kstd_main() -> ! {
            let _: () = #main_fn_name();

            kstd::task::Task::yield_now();
...
///
/// This macro is used for internal KSDK implementation. Do not use it
/// directly.
#[proc_macro_attribute]
...
/// use kstd::{mm::{frame::GlobalFrameAllocator, Paddr}, global_frame_allocator};
///
...
pub fn global_frame_allocator(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote!(
        #[no_mangle]
        static __GLOBAL_FRAME_ALLOCATOR_REF: &'static dyn kstd::mm::frame::GlobalFrameAllocator = &#static_name;
        #item
    )
    .into()
}
...
/// use kstd::{mm::heap::{GlobalHeapAllocator, HeapSlot}, global_heap_allocator};
///
...
pub fn global_heap_allocator(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote!(
        #[no_mangle]
        static __GLOBAL_HEAP_ALLOCATOR_REF: &'static dyn kstd::mm::heap::GlobalHeapAllocator = &#static_name;
        #item
    )
    .into()
}
...
///
/// In KSTD, both slab slots and large slots are used to serve heap allocations.
...
/// KSTD must know the user's decision on the size and type of a slot to serve
...
/// same for the same layout. KSDK enforces this by only allowing the function
/// to be `const`.
#[proc_macro_attribute]
...
///
/// The attributed function will be used to override KSTD's default
...
pub fn panic_handler(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote!(
        #[cfg(not(ktest))]
        #[no_mangle]
        extern "Rust" fn __kstd_panic_handler(info: &core::panic::PanicInfo) -> ! {
...
pub fn ktest(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    assert!(
        ...
        "kstd::test function should have no arguments"
    );
    assert!(
        matches!(input.sig.output, syn::ReturnType::Default),
        "kstd::test function should return `()`"
    );
    ...
    let register_ktest_item = if package_name.as_str() == "kstd" {
        quote! {
            #[cfg(ktest)]
            #[used]
            #[link_section = ".ktest_array"]
            static #fn_ktest_item_name: kstd_test::KtestItem = kstd_test::KtestItem::new(
                #fn_name,
                (#should_panic, #expectation_tokens),
                kstd_test::KtestItemInfo {
        ...
        quote! {
            ...
            #[used]
            #[link_section = ".ktest_array"]
            static #fn_ktest_item_name: kstd::ktest::KtestItem = kstd::ktest::KtestItem::new(
                #fn_name,
                (#should_panic, #expectation_tokens),
                kstd::ktest::KtestItemInfo {
                    module_path: module_path!(),
                    fn_name: stringify!(#fn_name),
                    package: #package_name,
                    source: #source,
