...
/// Calls the KSTD-user defined entrypoint of the actual kernel.
///
/// Any kernel that uses the `kstd` crate should define a function marked with
/// `kstd::main` as the entrypoint.
///
/// This function should be only called from the bootloader-specific module.
pub(crate) fn call_kstd_main() -> ! {
    ...
    // SAFETY: This external function is defined by the package that uses KSTD,
    // which should be generated by the `kstd::main` macro. So it is safe.
    unsafe {
        __kstd_main();
    }
}
