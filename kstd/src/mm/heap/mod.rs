...
/// mechanisms, KSTD allows users to implement their own kernel heap in a safe
...
pub trait GlobalHeapAllocator: Sync {
    ...
    ///
    /// KSTD calls this method to allocate memory from the global heap.
    ///
    ...
    ///
    /// KSTD calls this method to deallocate memory back to the global heap.
    ///
...
fn get_global_heap_allocator() -> &'static dyn GlobalHeapAllocator {
    // SAFETY: This up-call is redirected safely to Rust code by KSDK.
    unsafe { __GLOBAL_HEAP_ALLOCATOR_REF }
}

/// Gets the size and type of heap slots to serve allocations of the layout.
///
/// This function is defined by the KSTD user and should be idempotent, as we
