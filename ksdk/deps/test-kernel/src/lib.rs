...
//! The KSTD unit test runner is a kernel that runs the tests defined by the
//! `#[kstd::ktest]` attribute. The kernel should be automatically selected to
//! run when KSDK is used to test a specific crate.
...
#[kstd::ktest::main]
fn main() {
    use kstd::task::TaskOptions;

    let test_task = move || {
        use alloc::string::ToString;

        use kstd::arch::qemu::{exit_qemu, QemuExitCode};
...
#[kstd::ktest::panic_handler]
fn panic_handler(info: &core::panic::PanicInfo) -> ! {
    let _irq_guard = kstd::trap::disable_local();

    use alloc::{boxed::Box, string::ToString};

    use kstd::panic::begin_panic;

    let throw_info = kstd::ktest::PanicInfo {
