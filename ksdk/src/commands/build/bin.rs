...
use crate::{
    arch::Arch,
    bundle::{
        bin::{AstrosBin, AstrosBinType, AstrosBzImageMeta, AstrosElfMeta},
        file::BundleFile,
    },
...
pub fn make_install_bzimage(
    ...
    AstrosBin::new(
        &install_path,
        astros_elf.arch(),
        AstrosBinType::BzImage(AstrosBzImageMeta {
            support_legacy32_boot: linux_x86_legacy_boot,
            support_efi_boot: false,
            support_efi_handover: !linux_x86_legacy_boot,
        }),
        astros_elf.version().clone(),
        astros_elf.stripped(),
    )
}

pub fn make_elf_for_qemu(install_dir: impl AsRef<Path>, elf: &AstrosBin, strip: bool) -> AstrosBin {
    let result_elf_path = {
        let elf_name = elf
            .path()
            .file_name()
...
fn install_setup_with_arch(
    ...
    cmd.env("PAYLOAD_FILE", encode_kernel_to_file(astros_elf, encoding));
    cmd.arg("install").arg("linux-bzimage-setup");
    cmd.arg("--force");
    cmd.arg("--root").arg(install_dir.as_ref());
    if matches!(option_env!("KSDK_LOCAL_DEV"), Some("1")) {
        let crate_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let setup_dir = crate_dir.join("../kstd/libs/linux-bzimage/setup");
...
fn encode_kernel_to_file(astros_elf: &AstrosBin, encoding: PayloadEncoding) -> PathBuf {
    let kernel_path = astros_elf.path();
    let encoded_path = {
