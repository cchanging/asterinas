...
use crate::{
    bundle::{
        file::BundleFile,
        vm_image::{AstrosQcow2ImageMeta, AstrosVmImage, AstrosVmImageType},
    },
    error_msg,
};

pub fn convert_iso_to_qcow2(iso: AstrosVmImage) -> AstrosVmImage {
    let AstrosVmImageType::GrubIso(meta) = iso.typ() else {
    ...
    AstrosVmImage::new(
        qcow2_path,
        AstrosVmImageType::Qcow2(AstrosQcow2ImageMeta {
            grub_version: meta.grub_version.clone(),
        }),
        iso.astros_version().clone(),
    )
}
