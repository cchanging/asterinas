...
use crate::{
    ...
    bundle::{
        bin::{AstrosBin, AstrosBinType, AstrosElfMeta},
        file::BundleFile,
        Bundle,
    },
    cli::BuildArgs,
...
pub fn execute_build_command(config: &Config, build_args: &BuildArgs) {
    ...
    let ksdk_output_directory = build_args
        .output
        .clone()
        .unwrap_or(cargo_target_directory.join(DEFAULT_TARGET_RELPATH));
    if !ksdk_output_directory.exists() {
        std::fs::create_dir_all(&ksdk_output_directory).unwrap();
    }
    let target_info = get_current_crate_info();
    let bundle_path = ksdk_output_directory.join(target_info.name);
...
pub fn create_base_and_cached_build(
    ...
    ksdk_output_directory: impl AsRef<Path>,
    cargo_target_directory: impl AsRef<Path>,
    config: &Config,
    action: ActionChoice,
    rustflags: &[&str],
) -> Bundle {
    let base_crate_path = ksdk_output_directory.as_ref().join("base");
    new_base_crate(
        &base_crate_path,
...
pub fn do_cached_build(
    ...
    let astros_elf = build_kernel_elf(
        config.target_arch,
        &build.profile,
        &build.features[..],
    ...
    if let Some(existing_bundle) = get_reusable_existing_bundle(&bundle_path, config, action) {
        if astros_elf.modified_time() < &existing_bundle.last_modified_time() {
            info!("Reusing existing bundle: astros_elf is unchanged");
    ...
    match boot.method {
        BootMethod::GrubRescueIso | BootMethod::GrubQcow2 => {
            ...
            bundle.consume_astros_bin(astros_elf);
        }
        BootMethod::QemuDirect => {
            let qemu_elf = make_elf_for_qemu(&ksdk_output_directory, &astros_elf, build.strip_elf);
            bundle.consume_astros_bin(qemu_elf);
        }
    }
...
fn build_kernel_elf(
    ...
    // Astros does not support PIC yet.
    ...
    if matches!(arch, Arch::X86_64) {
        // This is a workaround for <https://github.com/astros/astros/issues/839>.
    ...
    let astros_bin_path = cargo_target_directory
        .as_ref()
    ...
    AstrosBin::new(
        astros_bin_path,
        arch,
        AstrosBinType::Elf(AstrosElfMeta {
