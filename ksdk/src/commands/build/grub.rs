...
use crate::{
    bundle::{
        bin::AstrosBin,
        file::BundleFile,
        vm_image::{AstrosGrubIsoImageMeta, AstrosVmImage, AstrosVmImageType},
    },
    config::{
...
pub fn create_bootdev_image(
    ...
    match protocol {
        ...
        _ => {
            ...
            hard_link_or_copy(astros_bin.path(), target_path).unwrap();
        }
    };
    ...
    AstrosVmImage::new(
        iso_path,
        AstrosVmImageType::GrubIso(AstrosGrubIsoImageMeta {
            grub_version: get_grub_mkrescue_version(&action.grub.grub_mkrescue),
        }),
        astros_bin.version().clone(),
    )
}
...
fn generate_grub_cfg(
    ...
    let astros_bin_path_on_device = PathBuf::from("/boot")
        .join(target_name)
        .into_os_string()
        .into_string()
        .unwrap();
    match protocol {
        BootProtocol::Multiboot => grub_cfg
            ...
            .replace("#KERNEL#", &astros_bin_path_on_device)
            .replace(
                "#GRUB_CMD_INITRAMFS#",
