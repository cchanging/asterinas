...
use crate::{
    ...
    error::Errno,
    error_msg,
    util::{cargo_new_lib, get_cargo_metadata, kstd_dep},
};
...
pub fn execute_new_command(args: &NewArgs) {
    ...
    create_ksdk_manifest(&cargo_metadata, &args.project_type());
    exclude_ksdk_base(&cargo_metadata);
    write_src_template(&cargo_metadata, &args.crate_name, &args.project_type());
    add_rust_toolchain(&cargo_metadata);
}

/// KSDK assumes that the toolchain used by the kernel should be same same as the toolchain
/// specified in the astros workspace.
fn astros_rust_toolchain() -> String {
...
fn add_manifest_dependencies(cargo_metadata: &serde_json::Value, crate_name: &str) {
    ...
    let kstd_dep = toml::Table::from_str(&kstd_dep()).unwrap();
    dependencies.as_table_mut().unwrap().extend(kstd_dep);

    let content = toml::to_string(&manifest).unwrap();
    fs::write(manifest_path, content).unwrap();
}

// Add `target/ksdk/base` and `target/ksdk/test-base` to `exclude` array of the workspace manifest
fn exclude_ksdk_base(metadata: &serde_json::Value) {
    let ksdk_run_base_path = "target/ksdk/base";
    let ksdk_test_base_path = "target/ksdk/test-base";
    ...
    if let Some(workspace) = manifest_toml.get_mut("workspace") {
        ...
        if let Some(exclude) = workspace.get_mut("exclude") {
            ...
            if exclude.contains(&toml::Value::String(ksdk_run_base_path.to_string()))
                || exclude.contains(&toml::Value::String(ksdk_test_base_path.to_string()))
            {
                return;
            }

            exclude.push(toml::Value::String(ksdk_run_base_path.to_string()));
            exclude.push(toml::Value::String(ksdk_test_base_path.to_string()));
        } else {
            let exclude = vec![
                toml::Value::String(ksdk_run_base_path.to_string()),
                toml::Value::String(ksdk_test_base_path.to_string()),
            ];
            workspace.insert("exclude".to_string(), toml::Value::Array(exclude));
        }
    } else {
        let exclude =
            toml::Table::from_str(r#"exclude = ["target/ksdk/base", "target/ksdk/test-base"]"#)
                .unwrap();
...
fn create_ksdk_manifest(cargo_metadata: &serde_json::Value, type_: &ProjectType) {
    let ksdk_manifest_path = {
        let workspace_root = get_workspace_root(cargo_metadata);
        PathBuf::from(workspace_root).join("KSDK.toml")
    };

    if ksdk_manifest_path.is_file() {
        // If the KSDK.toml of workspace exists, just return.
        return;
    }

    // Create `KSDK.toml` for the workspace
    let contents = match type_ {
        ProjectType::Kernel => {
            include_str!("kernel.KSDK.toml.template")
        }
        ProjectType::Library => {
            include_str!("lib.KSDK.toml.template")
        }
        ProjectType::Module => {
            todo!()
        }
    ...
    if is_in_virtual_workspace(cargo_metadata) {
        ...
        fs::write(ksdk_manifest_path, contents).unwrap();
        return;
    }
...
fn add_rust_toolchain(cargo_metadata: &serde_json::Value) {
    ...
    let contents = astros_rust_toolchain();
...
fn check_rust_toolchain(toolchain: &toml::Table) {
    ...
    if channel != expected_channel {
        error_msg!("The current version of rust-toolchain.toml is not compatible with the ksdk");
