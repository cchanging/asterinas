...
pub fn test_current_crate(config: &Config, args: &TestArgs) {
    ...
    let ksdk_output_directory = cargo_target_directory.join(DEFAULT_TARGET_RELPATH);
    // Use a different name for better separation and reusability of `run` and `test`
    let target_crate_dir = ksdk_output_directory.join("test-base");

    // A special case is that we use KSDK to test the KSDK test runner crate
    // itself. We check it by name.
    let runner_self_test = if current_crate.name == "ksdk-test-kernel" {
        if matches!(option_env!("KSDK_LOCAL_DEV"), Some("1")) {
            true
        } else {
            error_msg!("The tested crate name collides with the KSDK test runner crate");
...
"#,
    ...
    let default_bundle_directory = ksdk_output_directory.join(target_name);
