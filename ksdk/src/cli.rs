...
pub fn main() {
    ...
    let cli = Cli::parse();
    let CargoSubcommand::Ksdk(ksdk_subcommand) = &cli.cargo_subcommand;

    match ksdk_subcommand {
        KsdkSubcommand::New(args) => execute_new_command(args),
        KsdkSubcommand::Build(build_args) => {
            execute_build_command(&load_config(&build_args.common_args), build_args);
        }
        KsdkSubcommand::Run(run_args) => {
            execute_run_command(
                &load_config(&run_args.common_args),
                run_args.gdb_server.as_deref(),
            );
        }
        KsdkSubcommand::Debug(debug_args) => {
            execute_debug_command(
                &load_config(&debug_args.common_args).run.build.profile,
                debug_args,
            );
        }
        KsdkSubcommand::Profile(profile_args) => {
            execute_profile_command(
                &load_config(&profile_args.common_args).run.build.profile,
                profile_args,
            );
        }
        KsdkSubcommand::Test(test_args) => {
            execute_test_command(&load_config(&test_args.common_args), test_args);
        }
        KsdkSubcommand::Check(args) => execute_forwarded_command("check", &args.args, true),
        KsdkSubcommand::Clippy(args) => execute_forwarded_command("clippy", &args.args, true),
        KsdkSubcommand::Doc(args) => execute_forwarded_command("doc", &args.args, false),
    }
}
...
#[derive(Debug, Parser)]
pub enum KsdkSubcommand {
    #[command(about = "Create a new kernel package or library package which depends on KSTD")]
    New(NewArgs),
...
pub struct CommonArgs {
    ...
    #[arg(
        long = "scheme",
        help = "Select the specific configuration scheme provided in the KSDK manifest",
        value_name = "SCHEME",
        global = true
    )]
