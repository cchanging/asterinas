...
//! The base crate is the KSDK generated crate that is ultimately built by cargo.
...
fn do_new_base_crate(
    ...
    let cargo_toml = cargo_toml.replace("#NAME#", &(dep_crate_name.to_string() + "-ksdk-bin"));
    ...
    // Set the current directory to the target ksdk directory
...
fn add_manifest_dependency(
    ...
    add_manifest_dependency_to(dependencies, "kstd", Path::new("..").join("kstd"));
...
fn add_manifest_dependency_to(manifest: &mut toml::Value, dep_name: &str, path: PathBuf) {
    let dep_str = match option_env!("KSDK_LOCAL_DEV") {
        Some("1") => {
