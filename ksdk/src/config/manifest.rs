...
/// The ksdk manifest from configuration file `KSDK.toml`.
#[derive(Debug, Clone)]
...
impl TomlManifest {
    pub fn load() -> Self {
        ...
        // Search for KSDK.toml in the current directory first.
        let current_manifest_path = PathBuf::from("KSDK.toml").canonicalize();
        let current_manifest = match &current_manifest_path {
            ...
            Err(_) => {
                ...
                if let Ok(workspace_manifest_path) = workspace_root.join("KSDK.toml").canonicalize()
                {
        ...
        let Some(mut current_manifest) = current_manifest else {
            error_msg!("Cannot find `KSDK.toml` in the current directory or the workspace root");
    ...
    pub fn get_scheme(&self, scheme: Option<impl ToString>) -> &Scheme {
        if let Some(scheme) = scheme {
            ...
            if selected_scheme.is_none() {
                error_msg!("Scheme `{}` not found in `KSDK.toml`", scheme.to_string());
