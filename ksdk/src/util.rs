...
/// The version of KSTD on crates.io.
///
/// KSTD shares the same version with KSDK, so just use the version of KSDK here.
pub const KSTD_VERSION: &str = env!("CARGO_PKG_VERSION");
pub fn kstd_dep() -> String {
    format!("kstd = {{ version = \"{}\" }}", KSTD_VERSION)
}
...
///
/// A crate is considered a kernel crate if it utilizes the `kstd::main` macro.
    ...
    let packages: Vec<_> = {
        ...
        packages
            ...
            .filter(|package| {
                ...
                contains_kstd_main_macro(&file)
            })
            .collect()
    };

    if packages.is_empty() {
        error_msg!("KSDK requires there's at least one kernel package. Please navigate to the kernel package directory or the workspace root and run the command.");
        std::process::exit(Errno::BuildCrate as _);
    }

    if packages.len() >= 2 {
        error_msg!("KSDK requires there's at most one kernel package in the workspace. Please navigate to the kernel package directory and run the command.");
        std::process::exit(Errno::BuildCrate as _);
    }
...
fn contains_kstd_main_macro(file: &syn::File) -> bool {
    for item in &file.items {
        ...
        for attr in &item_fn.attrs {
            ...
            if attr.as_str() == "# [kstd :: main]" || attr.as_str() == "#[main]" {
                return true;
            }
        }
    }
...
pub fn trace_panic_from_log(qemu_log: File, bin_path: PathBuf) {
    ...
    for line in lines.into_iter().rev() {
        if line.contains("Printing stack trace:") {
            println!("[KSDK] The kernel seems panicked. Parsing stack trace for source lines:");
            trace_exists = true;
        }
        if trace_exists {
