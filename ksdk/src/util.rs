...
/// Information about an KSDK crate.
#[derive(Debug, Clone)]
pub struct CrateInfo {
    ...
    ///
    /// If a crate contains the `kstd::main` macro, it is a kernel crate (akin
...
pub fn get_current_crates() -> Vec<CrateInfo> {
    ...
    for member in default_members {
        ...
        let is_kernel_crate = package_contains_kstd_main(member_meta);
...
fn package_contains_kstd_main(package: &serde_json::Value) -> bool {
    let src_path = {
    ...
    file_contains_kstd_main_macro(&file)
}

fn file_contains_kstd_main_macro(file: &syn::File) -> bool {
