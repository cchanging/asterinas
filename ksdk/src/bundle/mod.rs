...
use std::process;
use vm_image::{AstrosVmImage, AstrosVmImageType};
...
/// The ksdk bundle artifact that stores as `bundle` directory.
///
...
/// The ksdk bundle artifact manifest that stores as `bundle.toml`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BundleManifest {
    pub initramfs: Option<Initramfs>,
    pub astros_bin: Option<AstrosBin>,
    pub vm_image: Option<AstrosVmImage>,
    pub config: Config,
...
impl Bundle {
    ...
    pub fn load(path: impl AsRef<Path>) -> Option<Self> {
        ...
        if let Some(astros_bin) = &manifest.astros_bin {
            if !astros_bin.validate() {
                return None;
            }
        }
    ...
    pub fn can_run_with_config(&self, config: &Config, action: ActionChoice) -> Result<(), String> {
        ...
        match config_action.boot.method {
            BootMethod::QemuDirect => {
                if self.manifest.astros_bin.is_none() {
            ...
            BootMethod::GrubRescueIso => {
                ...
                if !matches!(vm_image.typ(), AstrosVmImageType::GrubIso(_)) {
            ...
            BootMethod::GrubQcow2 => {
                ...
                if !matches!(vm_image.typ(), AstrosVmImageType::Qcow2(_)) {
    ...
    pub fn run(&self, config: &Config, action: ActionChoice) {
        ...
        match action.boot.method {
            BootMethod::QemuDirect => {
                let astros_bin = self.manifest.astros_bin.as_ref().unwrap();
                qemu_cmd
                    .arg("-kernel")
                    .arg(self.path.join(astros_bin.path()));
            ...
            BootMethod::GrubRescueIso => {
                ...
                assert!(matches!(vm_image.typ(), AstrosVmImageType::GrubIso(_)));
            ...
            BootMethod::GrubQcow2 => {
                let vm_image = self.manifest.vm_image.as_ref().unwrap();
                assert!(matches!(vm_image.typ(), AstrosVmImageType::Qcow2(_)));
        ...
        if let Ok(file) = std::fs::File::open(qemu_log_path) {
            if let Some(astros_bin) = &self.manifest.astros_bin {
                crate::util::trace_panic_from_log(file, self.path.join(astros_bin.path()));
            }
        }
        ...
        if !exit_status.success() {
            ...
            match kernel_exit_code {
                0x10 /*kstd::QemuExitCode::Success*/ => { std::process::exit(0); },
                0x20 /*kstd::QemuExitCode::Failed*/ => { std::process::exit(1); },
    ...
    pub fn consume_vm_image(&mut self, vm_image: AstrosVmImage) {
    ...
    /// Move the astros_bin into the bundle.
    pub fn consume_astros_bin(&mut self, astros_bin: AstrosBin) {
        if self.manifest.astros_bin.is_some() {
            panic!("astros_bin already exists");
        }
        self.manifest.astros_bin = Some(astros_bin.copy_to(&self.path));
        self.write_manifest_to_fs();
    }
