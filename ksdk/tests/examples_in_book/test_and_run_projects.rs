...
use crate::util::{cargo_ksdk, edit_config_files};
...
fn create_and_run_kernel() {
    ...
    let mut command = cargo_ksdk(&["new", "--kernel", os_name]);
    command.current_dir(work_dir);
    command.ok().unwrap();

    // Makes the kernel depend on local KSTD
    edit_config_files(&os_dir);

    let mut command = cargo_ksdk(&["build"]);
    command.current_dir(&os_dir);
    command.ok().unwrap();

    let mut command = cargo_ksdk(&["run"]);
    command.current_dir(&os_dir);
...
fn create_and_test_library() {
    ...
    let mut command = cargo_ksdk(&["new", module_name]);
    command.current_dir(work_dir);
    command.ok().unwrap();

    edit_config_files(&module_dir);

    let mut command = cargo_ksdk(&["test"]);
