...
use crate::util::{add_tdx_scheme, cargo_ksdk, depends_on_local_kstd, is_tdx_enabled};
...
fn work_in_workspace() {
    ...
    let kernel = "myos";
    let module = "mylib";
    cargo_ksdk(&["new", "--kernel", kernel])
        .current_dir(&workspace_dir)
        .ok()
        .unwrap();
    cargo_ksdk(&["new", module])
        .current_dir(&workspace_dir)
        .ok()
        .unwrap();
    ...
    // Make module depends on local kstd
    let module_manifest_path = workspace_dir.join(module).join("Cargo.toml");
    depends_on_local_kstd(module_manifest_path);
    ...
    depends_on_local_kstd(&kernel_manifest_path);

    if is_tdx_enabled() {
        add_tdx_scheme(workspace_dir.join("KSDK.toml")).unwrap();
    }
