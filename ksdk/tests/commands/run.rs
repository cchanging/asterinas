...
mod workspace {
    ...
    fn create_kernel_in_workspace(workspace: &str, kernel_name: &str) {
        ...
        let mut cargo_ksdk_new = cargo_ksdk(["new", "--kernel", kernel_name]);
        cargo_ksdk_new.current_dir(workspace);
        cargo_ksdk_new
            .ok()
    ...
    mod vsc {
        ...
        fn vsc_launch_file() {
            ...
            let bin_file_path = Path::new(&workspace.os_dir())
                ...
                .join("ksdk")
                .join(kernel_name)
                .join(format!("{}-ksdk-bin", kernel_name));
            let _gdb = std::thread::spawn(move || {
