...
# Publish KSDK and KSTD to crates.io
#
...
do_publish_for kstd/libs/linux-bzimage/boot-params
do_publish_for kstd/libs/linux-bzimage/builder
do_publish_for kstd/libs/linux-bzimage/setup \
    --target ../builder/src/x86_64-i386_pm-none.json \
    -Zbuild-std=core,alloc,compiler_builtins
do_publish_for ksdk

# All supported targets of KSTD, this array should keep consistent with
# `package.metadata.docs.rs.targets` in `kstd/Cargo.toml`.
TARGETS="x86_64-unknown-none"
for TARGET in $TARGETS; do
    do_publish_for kstd/libs/kstd-macros --target $TARGET
    do_publish_for kstd/libs/kstd-test --target $TARGET
    do_publish_for kstd --target $TARGET
    do_publish_for ksdk/deps/frame-allocator --target $TARGET
    do_publish_for ksdk/deps/test-kernel --target $TARGET

    # For actual publishing, we should only publish once. Using any target that
    # KSTD supports is OK. Here we use the first target in the list.
    if [ -z "$DRY_RUN" ]; then
        break
    fi
done
