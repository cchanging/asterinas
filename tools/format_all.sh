...
KERNEL_DEMO_FILE="$WORKSPACE_ROOT/ksdk/tests/examples_in_book/write_a_kernel_in_100_lines_templates/lib.rs"
if [ "$CHECK_MODE" = true ]; then
...
for CRATE in $EXCLUDED_CRATES; do
    ...
    case "$CRATE" in
        ...
        *cargo-component*)
            continue
            ;;
        # `target/ksdk/base` and `target/ksdk/test-base` are generated by KSDK and does not need to be formatted. 
        # The directories do not exist before running `make build` or `make test`.
        *target/ksdk/base*)
            continue
            ;;
        *target/ksdk/test-base*)
            continue
            ;;
    esac
