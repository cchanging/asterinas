...
verify "root subtree_control contains cpu" \
    "tr ' ' '\\n' < cgroup.subtree_control | grep -x cpu" \
    "cpu"
verify "root subtree_control contains memory" \
    "tr ' ' '\\n' < cgroup.subtree_control | grep -x memory" \
    "memory"
verify "root subtree_control contains pids" \
    "tr ' ' '\\n' < cgroup.subtree_control | grep -x pids" \
    "pids"
...
verify "child controllers contain cpu" \
    "tr ' ' '\\n' < cgroup.controllers | grep -x cpu" \
    "cpu"
verify "child controllers contain memory" \
    "tr ' ' '\\n' < cgroup.controllers | grep -x memory" \
    "memory"
verify "child controllers contain pids" \
    "tr ' ' '\\n' < cgroup.controllers | grep -x pids" \
    "pids"
