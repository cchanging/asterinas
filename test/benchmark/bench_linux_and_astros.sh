...
parse_raw_results() {
    ...
    jq --arg linux_result "${linux_result}" --arg astros_result "${astros_result}" \
        '(.[] | select(.extra == "linux_result") | .value) |= $linux_result |
         (.[] | select(.extra == "astros_result") | .value) |= $astros_result' \
...
generate_template() {
    ...
    local astros_legend=${legend//"{system}"/"Astros"}

    # Generate the result template JSON
    jq -n --arg linux "$linux_legend" --arg astros "$astros_legend" --arg unit "$unit" '[
        { "name": $linux, "unit": $unit, "value": 0, "extra": "linux_result" },
        { "name": $astros, "unit": $unit, "value": 0, "extra": "astros_result" }
    ]' > "${RESULT_TEMPLATE}"
}
...
run_benchmark() {
    local benchmark="$1"
    local run_mode="$2"
    local astros_scheme="$3"
    local smp="$4"

    echo "Preparing libraries..."
    prepare_libs

    # Set up Astros scheme if specified (Default: iommu)
    local astros_scheme_cmd="SCHEME=iommu"
    if [ -n "$astros_scheme" ]; then
        if [ "$astros_scheme" != "null" ]; then
            astros_scheme_cmd="SCHEME=${astros_scheme}"
        else
            astros_scheme_cmd=""
        fi
    fi

    # Prepare commands for Astros and Linux
    local astros_cmd="make run BENCHMARK=${benchmark} ${astros_scheme_cmd} SMP=${smp} ENABLE_KVM=1 RELEASE_LTO=1 NETDEV=tap VHOST=on 2>&1"
...
main() {
    ...
    if [[ -f "$bench_result" ]]; then
        astros_scheme=$(jq -r '.runtime_config.astros_scheme // ""' "$bench_result")
    else
        for job in "${BENCHMARK_ROOT}/${benchmark}"/bench_results/*; do
            [[ -f "$job" ]] && astros_scheme=$(jq -r '.runtime_config.astros_scheme // ""' "$job") && break
        done
    fi
    ...
    # Run the benchmark
    run_benchmark "$benchmark" "$run_mode" "$astros_scheme" "$smp"
