...
parse_raw_results() {
    ...
    yq --arg linux_result "${linux_result}" --arg astros_result "${astros_result}" \
...
generate_template() {
    ...
    yq -n --arg linux "$linux_legend" --arg aster "$astros_legend" --arg unit "$unit" '[
        { "name": $linux, "unit": $unit, "value": 0, "extra": "linux_result" },
        { "name": $aster, "unit": $unit, "value": 0, "extra": "astros_result" }
    ]' > "${RESULT_TEMPLATE}"
}
...
run_benchmark() {
    ...
    local smp_val=1
    local mem_val="8G"
    local astros_scheme_cmd_part="SCHEME=iommu" # Default scheme
    ...
    while IFS='=' read -r key value; do
         ...
         case "$key" in
             ...
             "astros_scheme")
                 if [[ "$value" == "null" ]]; then
                     astros_scheme_cmd_part="" # Remove default SCHEME=iommu
                 else
                     astros_scheme_cmd_part="SCHEME=${value}" # Override default
                 fi
                 ;;
             *)
    ...
    local astros_cmd="make run BENCHMARK=${benchmark} ${astros_scheme_cmd_part} SMP=${smp_val} MEM=${mem_val} ENABLE_KVM=1 RELEASE_LTO=1 NETDEV=tap VHOST=on 2>&1"
    ...
    astros_cmd=$(echo "$astros_cmd" | sed 's/^ *//;s/ *$//;s/  */ /g')
