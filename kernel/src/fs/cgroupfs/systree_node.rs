...
use crate::{
    fs::cgroupfs::controller::{Controller, ControllerType, SubCtrlState},
    prelude::*,
...
impl CgroupSystem {
    ...
    fn new() -> Arc<Self> {
        ...
        Controller::init_attr_set(&mut builder, true, true);
...
impl CgroupNode {
    pub(self) fn new(name: SysStr, depth: usize, sub_ctrl_state: SubCtrlState) -> Arc<Self> {
        ...
        Controller::init_attr_set(&mut builder, false, true);
...
inherit_sys_branch_node!(CgroupSystem, fields, {
    ...
    fn write_attr(&self, name: &str, reader: &mut VmReader) -> Result<usize> {
        match name {
            ...
            "cgroup.subtree_control" => {
                let (activate_ctrls, deactivate_ctrls, len) =
                    read_subtree_control_from_reader(reader)?;

                // Lock order: current controller -> child controllers
                let mut controller = self.controller.lock();
                for ctrl_type in activate_ctrls.iter_active() {
                    controller.activate(ctrl_type, self, None)?;
                }
                for ctrl_type in deactivate_ctrls.iter_active() {
    ...
    fn write_attr(&self, name: &str, reader: &mut VmReader) -> Result<usize> {
        match name {
            ...
            "cgroup.subtree_control" => {
                ...
                // Lock order: parent controller -> current controller -> child controllers
...
/// along with the number of bytes read. The activeate and deactivate sets
/// will not overlap.
fn read_subtree_control_from_reader(
    reader: &mut VmReader,
) -> Result<(SubCtrlState, SubCtrlState, usize)> {
    let (content, len) = reader
        .read_cstring_until_end(MAX_ATTR_SIZE)
        .map_err(|_| Error::PageFault)?;
    let context = content.to_str().map_err(|_| Error::InvalidOperation)?;

    let mut activate_ctrls = SubCtrlState::empty();
    let mut deactivate_ctrls = SubCtrlState::empty();
    ...
    for action in actions {
        ...
        match action.chars().next() {
            Some('+') => {
                let ctrl_type = ControllerType::try_from(&action[1..])?;
                activate_ctrls |= SubCtrlState::control_bit(ctrl_type);
                deactivate_ctrls -= SubCtrlState::control_bit(ctrl_type);
            }
            Some('-') => {
                let ctrl_type = ControllerType::try_from(&action[1..])?;
                deactivate_ctrls |= SubCtrlState::control_bit(ctrl_type);
    ...
    Ok((activate_ctrls, deactivate_ctrls, len))
}
