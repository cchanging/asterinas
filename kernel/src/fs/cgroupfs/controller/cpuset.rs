...
/// The controller responsible for CPU resource management in the cgroup subsystem.
...
impl CpuSetController {
    pub(super) fn new(ctrl_state: super::SubCtrlState, is_root: bool) -> Option<Arc<Self>> {
        if !ctrl_state.contains(super::SubCtrlState::CPUSET_CTRLS) {
            return None;
        }

        let mut builder = SysAttrSetBuilder::new();
        Self::init_attr_set(&mut builder, is_root);
