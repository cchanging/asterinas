...
use crate::{
    fs::cgroupfs::{
        ...
        systree_node::with_process_cgroup_locked,
        CgroupNode,
    },
    prelude::*,
        ...
        match name {
            ...
            "cgroup.procs" => {
                if let Some(cgroup_node) = cgroup_node.as_any().downcast_ref::<CgroupNode>() {
                    writeln!(printer, "{}", cgroup_node.read_procs())?;
                } else {
            ...
            "cgroup.events" => {
                ...
                let res = if cgroup_node.populated_count().load(Ordering::Relaxed) > 0 {
                    1
                } else {
                    0
                };
            ...
            "cgroup.procs" => {
                ...
                if let Some(cgroup_node) = cgroup_node.as_any().downcast_ref::<CgroupNode>() {
                    with_process_cgroup_locked(pid, |process, cgroup_membership| {
                        ...
                        cgroup_membership.move_process_to_node(process, cgroup_node);
