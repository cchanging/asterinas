...
impl IdBitmap {
    ...
    /// # Panics
    ///
    /// This method panics if `len` is greater than `IdBitmap::max_len()`.
    pub fn from_buf(buf: Box<[u8]>, len: u16) -> Self {
        ...
        bitmap.first_available_id = (0..len)
            .find(|&i| !bitmap.bit_slice()[i as usize])
            .unwrap_or(len);
        bitmap
    }
    ...
    /// Returns the maximum length of an ID bitmap.
    pub const fn max_len() -> u16 {
        BLOCK_SIZE as u16 * 8
    }
    ...
    pub fn alloc(&mut self) -> Option<u16> {
        if self.first_available_id < self.len {
            ...
            self.first_available_id = (id + 1..self.len)
                .find(|&i| !self.bit_slice()[i as usize])
                .unwrap_or(self.len);
            Some(id)
        } else {
            None
