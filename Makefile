...
build: initramfs $(CARGO_KSDK)
	@cargo ksdk build $(CARGO_KSDK_ARGS)
...
run: initramfs $(CARGO_KSDK)
	@cargo ksdk run $(CARGO_KSDK_ARGS)
...
gdb_server: initramfs $(CARGO_KSDK)
	@cargo ksdk run $(CARGO_KSDK_ARGS) --gdb-server wait-client,vscode,addr=:$(GDB_TCP_PORT)

.PHONY: gdb_client
gdb_client: initramfs $(CARGO_KSDK)
	@cargo ksdk debug $(CARGO_KSDK_ARGS) --remote :$(GDB_TCP_PORT)

.PHONY: profile_server
profile_server: initramfs $(CARGO_KSDK)
	@cargo ksdk run $(CARGO_KSDK_ARGS) --gdb-server addr=:$(GDB_TCP_PORT)

.PHONY: profile_client
profile_client: initramfs $(CARGO_KSDK)
	@cargo ksdk profile $(CARGO_KSDK_ARGS) --remote :$(GDB_TCP_PORT) \
...
ktest: initramfs $(CARGO_KSDK)
	...
	@for dir in $(KSDK_CRATES); do \
		...
		(cd $$dir && OVMF=off cargo ksdk test) || exit 1; \
